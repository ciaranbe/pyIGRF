...
def option3():
    ...
    while 1:
        print(
            "Enter starting latitude, increment/decrement and "
        ...
        if abs(lati) > (abs(lats - late)):
            print(
                "Increment or decrement are larger than the gap between the "
    ...
    while 1:
        print(
            "Enter starting longitude, increment/decrement and"
        ...
        if lons < -180 or lons > 360 or lone < -180 or lone > 360:
            continue
    ...
    colat, lon = np.meshgrid(
        90 - np.arange(lats, late, lati), np.arange(lons, lone, loni)
    )
...
def write1(
    ...
    if not name:  # Print to screen
        print(
            "\nGeomagnetic field values at: ",
            str(np.round(lat, decimals=4))
            + degree_sign
            + " / "
            + str(lon)
            + degree_sign
        ...
        with open(name, "w") as file:
            ...
            file.writelines(
                ["Declination (D):", "{: 5.2f}".format(dec), degree_sign + "\n"]
            )
            file.writelines(
                ["Inclination (I):", "{: 5.2f}".format(inc), degree_sign + "\n"]
            )
            file.writelines(
                ["Horizontal intensity (H):", "{: 8.1f}".format(hoz), "nT\n"]
            )
            file.writelines(
                ["Total intensity (F)     :", "{: 8.1f}".format(eff), "nT\n"]
            )
            ...
            file.writelines(
                ["Declination SV (D) :", "{: 5.2f}".format(decs), "arcmin/yr\n"]
            )
            file.writelines(
                ["Inclination SV (I) :", "{: 5.2f}".format(incs), "arcmin/yr\n"]
            )
            file.writelines(
                ["Horizontal SV (H)  :", "{: 7.1f}".format(hozs), "nT/yr\n"]
            )
            file.writelines(
                ["Total SV (F)       :", "{: 7.1f}".format(effs), "nT/yr\n"]
            )
...
def write2(
    ...
    if not name:  # Print to screen
        ...
        print(
            "Date  D(" + degree_sign + ")  I(" + degree_sign + ")  H(nT)"
            " F(nT) X(nT) Y(nT)  Z(nT)     "
...
def write3(
    ...
    if not name:  # Print to screen
        print(
            "\nGeomagnetic field values for: ",
            str(date[0]) + ", at altitude " + str(np.round(alt[0], decimals=3)),
        )
        print(
            "Latitude  Longitude  D(" + degree_sign + ")  I(" + degree_sign + ")  H(nT)"
            " F(nT) X(nT) Y(nT)  Z(nT)     "
            "SV_D(min/yr)  SV_I(min/yr)  SV_H(nT/yr) "
            " SV_F(nT/yr)  SV_X(nT/yr)  SV_Y(nT/yr)  SV_Z(nT/yr) "
        )
